...
/**
 * Make an App-to-User transaction
 *
 * This `{ func }` makePaymentToUser will send Pi from the app to the user. 
 *
 * @param {Object} data  Receive `{ data }`, and creates payments with the given parameters.
 * 
 * `Example : Options`
 ...
 * @return {Object} `{ payment }`
 */
PieCard.prototype.makePaymentToUser = async function (data) {
  const clientValidation = this._validateClientCredentials();
  ...
  if (clientValidation) return clientValidationMessage;
    ...
    const { clientid, clientsecret, accesstoken } = this;
    const { amount, recipient, wallet_public_key, wallet_secret_key, memo } =
      data;

    const verifiedWallet = await httpReq({
    ...
    if (!verifiedWallet?.data?.isValid)
