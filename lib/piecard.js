 ...
 *
 * This `{ func } validate is used to retrieve data securely from an incoming webhook.
 *
 * @param {Object} encryptedData  Receive { body }, and decodes the request body in the webhook.
 *
 * @return {Object} `{ event_type, { payment } }`
 */
 PieCard.prototype.validate = async function (encryptedData) {
  const isValidJwt = isJWT(encryptedData);
  if (isValidJwt) {
    const data = jwt.verify(encryptedData, this.clientsecret);
    return data;
